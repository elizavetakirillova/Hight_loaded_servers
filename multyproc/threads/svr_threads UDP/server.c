...
void* server_thread(void *param)
{
	...
	switch(vars->sock_type) {	// по протоколу
	    case SOCK_STREAM:{	// TCP proto
		// принять данные от клиента
		if ((in_size = recv(client_sock, (void *)buf, BUF_SIZE, 0)) > 0) {
		    ...
		    if ((out_size = send(client_sock, (void *)buf, sizeof(*buf), 0)) > 0) {
	    ...
	    case SOCK_DGRAM:{	// UDP proto
		fprintf(stderr, "UDP type socket in main thread!\n");
		res = 0;
		val->in_size = 0;
		val->out_size = 0;
		break;
...
int main(int argc, char **argv, char **env)
{
	...
	case SOCK_DGRAM:{	// UDP
	    ...
	    svr_addr.sin_family = AF_INET;
	    svr_addr.sin_port = htons(svr_port);
	    svr_addr.sin_addr.s_addr = inet_addr("0.0.0.0");	// на всех сетевых интерфейсах
	    break;
	};
    };
    ...
    if ((sock_type == SOCK_STREAM) && (listen(svr_socket, 5)) == -1) {	// слушать сокет, 5 клиентов по умолчанию очередь!
	fprintf(stderr, "listen() error!\n");
	close(svr_socket);
	return 1;
    }
    fprintf(stderr, "Server listen on: 0.0.0.0:%d, proto: %s...\n",
	svr_port, argv[2]);
    // главный цикл сервера TCP
    if (sock_type == SOCK_STREAM) {
	while(res) {
	    clt_addr = (struct sockaddr_in*)malloc(sizeof(struct sockaddr_in));
	    if (!clt_addr) {
		fprintf(stderr, "TCP proto: error alloc mem for struct client_addr!\n");
		threadlist_rmall();
		close(svr_socket);
		res = 0;
		exit(1);
	...
	clt_addr = (struct sockaddr_in*)malloc(sizeof(struct sockaddr_in));
	if (!clt_addr) {
	    fprintf(stderr, "UDP proto: error alloc mem for struct client_addr!\n");
	    threadlist_rmall();
	    close(svr_socket);
	    res = 0;
	    exit(1);
	...
	// основной цикл
	while(res) {
	    // очистить адрес клиента
	    memset(clt_addr, 0, sizeof(struct sockaddr_in));
	    if ((in_size = recvfrom(svr_socket, (void *)&buf, BUF_SIZE, 0,
		(struct sockaddr *)clt_addr, (socklen_t *)sizeof(struct sockaddr_in))) > 0) {
		fprintf(stderr, "Received data from IP: %s, port: %lu.", inet_ntoa(clt_addr->sin_addr), ntohs(clt_addr->sin_port));
		fprintf(stderr, "Incoming msg: %s, size: %lu bytes.\n",
		    buf, in_size);
		if ((strcasecmp(buf, "Hi") == 0) || (strcasecmp(buf, "Hello"))) {
		    // чистим буфер
		    memset(&buf, '\0', BUF_SIZE);
		    ...
		    if ((out_size = sendto(svr_socket, (void *)&buf, strlen(buf), 0,
			(struct sockaddr *)clt_addr, sizeof(struct sockaddr_in))) > 0) {
			fprintf(stderr, "sendto(): sent data out %s, size: %lu bytes.\n",
			    buf, out_size);
		    } else {
			fprintf(stderr, "sendto(): error sent data!\n");
		    }
		    close(svr_socket);
		    res = 0;
		    free(clt_addr);
			...
			fprintf(stderr, "sendto(): send error!\n");
		    }
		    close(svr_socket);
		    res = 0;
		    free(clt_addr);
